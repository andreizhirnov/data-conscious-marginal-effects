...
margins, dydx(neweduc) at(neweduc=`neweduc_s' closing=`closing_s' (mean) _continuous (base) _factor) saving(temp_bg,replace)
...
margins, dydx(neweduc) over(group_id) at((omean) _continuous (base) _factor (mean) neweduc closing) saving(temp_me,replace)

collapse (count) obs=neweduc (mean) neweduc closing, by(group_id)
...
twoway (contour me_est neweduc closing if me_est!=., ccuts(`ccuts') ccolors(`colr')) ///
...
xtitle(Closing Date) ytitle(Education) ztitle("") zlabel(`zlabs') ///
